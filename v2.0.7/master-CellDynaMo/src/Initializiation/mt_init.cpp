/*
 * MTs initialization module
 *
 *  Created on: Mar 15, 2018
 *	Edited on:	Apr 21, 2021
 *      Author: kliuchnikov
 */
...
void MT_seeds(MT* pol[], int* type, float* length){
	...
	for (j = 0; j < POLE_COUNT; j++){
		for (m = 0; m < MT_NUM; m++){
			...
			pol[j][m].length = (START_LENGTH - MIN_MT_LENGTH)*ran2(&rseed) + MIN_MT_LENGTH;
	...
	for (j = 0; j < POLE_COUNT; j++){
		for (m = 0; m < MT_NUM; m++){
			...
			for (mi = 0; mi < MAX_MT_LENGTH; mi++){
				pol[j][m].r[mi].x = pol_center[j].x + (POLE_RADIUS + mi*pol[j][m].length/(MAX_MT_LENGTH - 1))*sin(theta)*cos(phi);
				pol[j][m].r[mi].y = pol_center[j].y + (POLE_RADIUS + mi*pol[j][m].length/(MAX_MT_LENGTH - 1))*sin(theta)*sin(phi);
				pol[j][m].r[mi].z = pol_center[j].z + (POLE_RADIUS + mi*pol[j][m].length/(MAX_MT_LENGTH - 1))*cos(theta);
				int i = j*MT_NUM*MAX_MT_LENGTH + m*MAX_MT_LENGTH + mi;
				length[i] = pol[j][m].length/(MAX_MT_LENGTH - 1);
			}
			float mindist = 100.0;
			int m1;
