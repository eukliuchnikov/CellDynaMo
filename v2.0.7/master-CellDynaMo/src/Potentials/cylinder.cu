/*
 *cylinder potential module
 *
 *  Created on: Apr 02, 2020
 *	Edited on:	Mar 16, 2021
 *      Author: kliuchnikov
 */

#include "cylinder.cuh"		

__global__ void cyl_cyl(float3* r, float3* f, int N, int* type, int kn, Param* d_parameters){
	...
	if (i < N_pol*N_mt*N_mt_max){
        if (type[i] == MT_REG || type[i] == LEFT_MINUS || type[i] == RIGHT_MINUS){
		    float3 ri1 = r[i];  //a0
		    float3 rj1 = r[i + 1];  //a1
            ...
            for (int p = 0; p < N_kt; p++){
                int js = N_pol*N_mt*N_mt_max + (p + 1)*kn - 1 - chrom_bn;
                float3 ri2 = r[js]; //b0
                if (type[js] == LEFT_KT){
                    float3 rj2 = r[js + kn];  //b1
                    ...
                    if (min_dist < 3.0*mt_rad + kt_rad){
                        ...
                        float df = -6*100*e_lj*powf(((mt_rad + kt_rad)/min_dist), 12)*powf(min_dist, -1)*powf(min_dist, -1);
                        if (df > 100.0){
                            df = 100.0;
                        } 
                        if (df < -100.0){
                        ...
                        //printf("KT_CHROM: %d\t%d\t%d\t%d\t%f\t%f\t%f\n", i, i+1, js, js + 1 + shift*chrom_bn/2, df, min_dist, mt_rad + kt_rad);
                        ...
                        atomicAdd(&f[js].x, -df*(1 - betha)*dx);
                        atomicAdd(&f[js].y, -df*(1 - betha)*dy);		
                        atomicAdd(&f[js].z, -df*(1 - betha)*dz); 

                        atomicAdd(&f[js + kn].x, -df*betha*dx);
                        atomicAdd(&f[js + kn].y, -df*betha*dy);  		
                        atomicAdd(&f[js + kn].z, -df*betha*dz);
                    }
                }
                for (int shift = 0; shift < 2; shift++){
	                float3 rj2 = r[js + 1 + shift*chrom_bn/2];  //b1
                    float3 p_vector2, p2_norm;
                    p_vector2.x = rj2.x - ri2.x; //B
                    p_vector2.y = rj2.y - ri2.y;
                    p_vector2.z = rj2.z - ri2.z;
                    ...
                    if (min_dist < mt_rad + kt_rad){
                        ...
                        atomicAdd(&f[js + 1 + shift*chrom_bn/2].x, -df*betha*dx);
                        atomicAdd(&f[js + 1 + shift*chrom_bn/2].y, -df*betha*dy);  		
                        atomicAdd(&f[js + 1 + shift*chrom_bn/2].z, -df*betha*dz);
                    }
                }
                for (int ch = 2; ch <= (chrom_bn/2); ch++){
                    int j = N_pol*N_mt*N_mt_max + (p + 1)*kn - ch;
                    ri2 = r[j]; //b0
		            float3 rj2 = r[j + 1];  //b1
                    ...
                    if (min_dist < 3.0*mt_rad + kt_rad){
                        ...
                        //printf("CHROM1: %d\t%d\t%d\t%d\t%f\t%f\t%f\n", i, i+1, j, j + 1, df, min_dist, mt_rad + kt_rad);
                        atomicAdd(&f[i].x, df*(1 - alpha)*dx);
                        atomicAdd(&f[i].y, df*(1 - alpha)*dy);  		
                        atomicAdd(&f[i].z, df*(1 - alpha)*dz);
                ...
                for (int ch = chrom_bn/2 + 2; ch <= chrom_bn; ch++){
                    int j = N_pol*N_mt*N_mt_max + (p + 1)*kn - ch;
                    float3 ri2 = r[j]; //b0
		            float3 rj2 = r[j + 1];  //b1
	...
	if (i < 2*N_pol*N_mt*N_mt_max){
        if (type[i] == MT_REG || type[i] == MT_END || type[i] == LEFT_MINUS || type[i] == RIGHT_MINUS){
            ...
            /*for (int h = 0; h < N_kt; h++){
                ...
                if (min_dist < 2*mt_rad + kt_rad && occup == 0){
                    float3 d_a;
                   ...
                   // float df = -6*e_lj*powf(((mt_rad + kt_rad)/min_dist), 12)*powf(min_dist, -1)*powf(min_dist, -1);
                    float df = -6*10000.0*powf(((mt_rad + kt_rad)/min_dist), 12)*powf(min_dist, -1)*powf(min_dist, -1);
                    if (df > 100.0){
                        df = 100.0;
                    } 
                    if (df < -100.0){
            ...
            for (int p = 0; p < N_kt; p++){
                ...
                for (int sh = x_num*y_num; sh < kn; sh++){
                    ...
                    if (min_dist < 2*mt_rad + 60.0 && occup == 0){
                        float3 d_a;
                        d_a.x = intersect.x - ri1.x;
                        d_a.y = intersect.y - ri1.y;
                        d_a.z = intersect.z - ri1.z;
                        ...
                        if (p % 2 == 0){
	                        ...
	                        float dx1 = kt1.x - intersect.x;
		                    float dy1 = kt1.y - intersect.y;
		                    float dz1 = kt1.z - intersect.z;

	                        float dx2 = kt2.x - intersect.x;
		                    float dy2 = kt2.y - intersect.y;
		                    float dz2 = kt2.z - intersect.z;
