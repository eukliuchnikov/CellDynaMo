	...
	for (h = 0; h < KIN_COUNT; h++){
        ...
        while (NDCcounter > 0){
            for (i = 0; i < kt[h].n; i++){
                float c = ran2(&rseed);
                //printf("%f\t%f\n", c, 20.0/kt[h].n);
                if (c < 20.0/kt[h].n && kt[h].attach[i] != DETACH && NDCcounter > 0){
                    //printf("%d\t%d\tsuccess\n", h, i);
			...
			if (KT_SHAPE == KT_RECT){
				...
				for (theta = 0; theta < 2*M_PI; theta += 0.025*2*M_PI){
					for (phi = 0; phi < 2*M_PI; phi += 0.025*2*M_PI){
						kin coord;
						coord.x = -2*KIN_RADIUS*sin(theta)*cos(phi);
						coord.y = (2*KIN_RADIUS)*sin(theta)*sin(phi);
						coord.z = (2*KIN_RADIUS)*cos(theta);
						coord.w = NO_NDC;
