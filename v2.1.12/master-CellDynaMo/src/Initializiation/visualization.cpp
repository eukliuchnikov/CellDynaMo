...
int initPSF(KT* kt){
	...
	psf.nbond = POLE_COUNT*MT_NUM*(MAX_MT_LENGTH - 1) + KIN_COUNT*(kt[0].n - 2) + KIN_COUNT/2 + POLE_COUNT*MT_NUM;
	...
	for (i = 0; i < psf.natom; i++){
		...
		if (i < POLE_COUNT*MT_NUM*MAX_MT_LENGTH){
			...
			for (int h = 0; h < POLE_COUNT; h++){	
				for (int k = 0; k < MT_NUM; k++){
					if (i == POLE_COUNT*MT_NUM*MAX_MT_LENGTH + KIN_COUNT*kn + 2*h*MT_NUM + k*2){
						sprintf(psf.atoms[i].name, "KMT");
						sprintf(psf.atoms[i].type, "KMT");
						sprintf(psf.atoms[i].segment, "KMT");
					}
					if (i == POLE_COUNT*MT_NUM*MAX_MT_LENGTH + KIN_COUNT*kn + 2*h*MT_NUM + k*2 + 1){
						sprintf(psf.atoms[i].name, "KCH");
						sprintf(psf.atoms[i].type, "KCH");
						sprintf(psf.atoms[i].segment, "KCH");
					}
				}
			}
		}
...
//MT's bonds
	...
	for (int h = 0; h < POLE_COUNT; h++){	
		for (int k = 0; k < MT_NUM; k++){
			psf.bonds[b].i = POLE_COUNT*MT_NUM*MAX_MT_LENGTH + KIN_COUNT*kn + 2*h*MT_NUM + k*2 + 1;
			psf.bonds[b].j = POLE_COUNT*MT_NUM*MAX_MT_LENGTH + KIN_COUNT*kn + 2*h*MT_NUM + k*2 + 2;
			b++;
		}
	}
	b--;
