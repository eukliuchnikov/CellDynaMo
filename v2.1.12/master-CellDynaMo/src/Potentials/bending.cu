...
__global__ void computeAngles(float3* r, float3* f, int N, int* type, int kn, float* kt_cos, float* dyn_cos, Param* d_parameters, float* length, int* connect1, int* connect2){
	int i;
	...
	if (i < N){
		if (type[i] == MT_REG){
           // float k_theta = 100*KB*temper*pers_l/(length[i]/(N_mt_max - 1));
            float k_theta = 100*KB*temper*pers_l/1000;
            if (length[i]/(N_mt_max - 1) > 12*mt_rad){
                float theta0 = M_PI;
                int j = i + 1;
                int k = i - 1;
			    float3 r1 = r[k];
        ...
        if (type[i] == LEFT_MINUS || type[i] == RIGHT_MINUS){
            float k_theta = 100*KB*temper*pers_l/(length[i]/(N_mt_max - 1));
            if (length[i]/(N_mt_max - 1) > 12*mt_rad){
                float theta0 = M_PI;
                int j = i + 1;
