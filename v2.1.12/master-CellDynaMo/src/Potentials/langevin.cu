...
//MT dynamics
		if (type[i] == MT_REG || type[i] == PLUS_DET || type[i] == PLUS_ATT || type[i] == PLUS_DET_INVALID || type[i] == LIG_MT_ACTIVE || type[i] == LIG_CH_ACTIVE){
...
__global__ void integrateGPU_init(float3* r, float3* f, int N, int* type, int kn, Param* d_parameters){
	...
	if (i < N){
        ...
        //float kt_var = sqrtf(KB*temperatur*2.0*time_step*pow(kt_gamma, -1));
        float ktc_gamma = 6*M_PI*viscos*d_parameters->kt_r;
        float ktc_var = sqrtf(KB*10*temperatur*2.0*time_step*pow(ktc_gamma, -1));
		if (type[i] == LEFT_NDC || type[i] == RIGHT_NDC){
            ...
            if (f[i].x > 100000000.0){
                f[i].x = 100000000.0;
            }
            if (f[i].y > 100000000.0){
                f[i].y = 100000000.0;
            }
            if (f[i].z > 100000000.0){
                f[i].z = 100000000.0;
            }
            if (f[i].x < -100000000.0){
                f[i].x = -100000000.0;
			...
			r[i].x += f[i].x*time_step*10*pow(ktc_gamma, -1) + ktc_var*rf.x;/* + ktc_var*rf.x*///;
			r[i].y += f[i].y*time_step*10*pow(ktc_gamma, -1) + ktc_var*rf.y;
            r[i].z += f[i].z*time_step*10*pow(ktc_gamma, -1) + ktc_var*rf.z;
		}
        f[i].x = 0.0f;
    	f[i].y = 0.0f;
	    f[i].z = 0.0f;
...
void dynamics(DCD dcd, float time){
    ...
    cudaMemcpy(mds.d_connect_1, mds.h_connect_1, mds.N*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(mds.d_connect_2, mds.h_connect_2, mds.N*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(mds.d_len_1, mds.h_len_1, mds.N*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(mds.d_len_2, mds.h_len_2, mds.N*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(mds.d_f_out, mds.h_f_out, mds.N*sizeof(float), cudaMemcpyHostToDevice);
    for (step = 0; step < 20; step++){
        ...
        //printf("Harmonic\n");       
        computeHarmonic<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, rk, d_harmonicKinCount, d_harmonicKin, d_harmonicKinRadii, d_kin_rad, d_parameters, mds.d_length, mds.d_connect_1, mds.d_connect_2, mds.d_len_1, mds.d_len_2, mds.d_f_out);
        cudaMemcpy(mds.h_f_out, mds.d_f_out, mds.N*sizeof(float), cudaMemcpyDeviceToHost);
        //printf("Bending\n");
        computeAngles<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_kin_cos, d_dyn_cos, d_parameters, mds.d_length, mds.d_connect_1, mds.d_connect_2);
        ...
        cyl_cyl<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
        //cyl_sphere<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
        //printf("Pulling\n");
        ...
        cudaMemcpy(mds.h_type, mds.d_type, KIN_COUNT*kn*sizeof(int), cudaMemcpyDeviceToHost);
        checkCUDAError();
	}
}
...
void minimiz(DCD dcd){
    ...
    for (step = 0; step < 30000; step++){
        ...
        pushing<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, step, mds.d_att_n, mds.d_att_f, d_parameters);
        excl_vol_kt<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
        chrom_ter<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
    ...
    for (step = 0; step < 100000; step++){
        ...
        cudaMemcpy(mds.d_att_f, mds.h_att_f, mds.N*sizeof(float), cudaMemcpyHostToDevice);   
        excl_vol_kt<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
        chrom_memb<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
		integrateGPU_init<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, kn, d_parameters);
        cudaMemcpy(mds.h_f, mds.d_f, mds.N*sizeof(float3), cudaMemcpyDeviceToHost);
        cudaMemcpy(mds.h_r, mds.d_r, mds.N*sizeof(float3), cudaMemcpyDeviceToHost);
        cudaMemcpy(mds.h_length, mds.d_length, mds.N*sizeof(float), cudaMemcpyDeviceToHost);
