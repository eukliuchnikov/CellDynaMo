...
//MT dynamics
		if (type[i] == MT_REG || type[i] == PLUS_DET || type[i] == PLUS_ATT || type[i] == PLUS_DET_INVALID || type[i] == MT_END){
...
void dynamics(DCD dcd, float time){
    ...
    for (step = 0; step < 20; step++){
        ...
        printf("Pushing\n");
        cyl_cyl<<<blockNum, blockSize>>>(mds.d_r, mds.d_f, mds.N, mds.d_type, d_parameters);
