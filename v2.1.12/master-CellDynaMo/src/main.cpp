		...
		if (minimiz == 1){
			...
			for (int m = 0; m < KIN_COUNT; m += 2){
                ...
                if (null.x > num_x_cell || null.y > num_y_cell || null.z > num_z_cell || null.x <= 0 || null.y <= 0 || null.z <= 0){
                    printf("main1: %d\n", POLE_COUNT*MT_NUM*MAX_MT_LENGTH + (m + 1)*kt[0].n - n_chrom - 1);
                }
				int n = COORintoARRAY(null.x, null.y, null.z);
				SV_centers_before[number] = n;
			}		
