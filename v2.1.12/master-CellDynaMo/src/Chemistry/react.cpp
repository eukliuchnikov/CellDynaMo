...
void reaction(float time, FILE* outp_att, int* Nab, int* Naa, 
	int* Nn0, int* Nn1, int* Nn2, int* Nn3, int* Nn4, int* Nn5, int* Nn6, int* Nn7, 
	int* Nn0_A, int* Nn1_A, int* Nn2_A, int* Nn3_A, int* Nn4_A, int* Nn5_A, int* Nn6_A, int* Nn7_A, 
	int* Nmtpol, int* Nmtdepol, int* Nmtdet, int* kinesin_att, int* kinesin_det, int & minimiz){
	...
	c[35] = K_ATTACH*trans/(sv_size*sv_size*sv_size);		
	c[36] = K_DETACH_0;
	c[37] = 12.0;
	...
	for (i = 1; i < num_x_cell + 1; i++){
		for (j = 1; j < num_y_cell + 1; j++){
			for (k = 1; k < num_z_cell + 1; k++){
				...
				hi[35] = kinesin_det[l];					//phos1
				hi[36] = kinesin_att[l];					//phos2
				hi[37] = kinesin_att[l];
				
				int r_id;
				float alpha0 = 0;
				for (r_id = 1; r_id < REACT_N; r_id++){
...
///////////////////////////////////////////
					if (mu == 15){
						GROWTHfunc(l, mds.h_r, mds.h_type, minimiz, mds.h_length, mds.h_connect_1, mds. h_connect_2, mds.h_len_1, mds.h_len_2, mds.h_att_l);
					}
					if (mu == 16){
						SHORTfunc(l, mds.h_r, mds.h_type, minimiz, mds.h_length, mds.h_att_l, mds.h_connect_1, mds. h_connect_2, mds.h_len_1, mds.h_len_2, mds.h_dom_center);
					}
					if (mu == 17){
...
///////////////////////////////////////////
					if (mu == 19){
						int attType = DETACH;
						int reactType = NDC_0; 
						AttachDettach(attType, outp_att, l, mds.h_r, mds.h_type, mu, reactType, time, mds.h_connector, mds.h_att_l, mds.h_length, mds.h_link_l);
					}
					if (mu == 20){
						int attType = DETACH;
						int reactType = NDC_1; 
...
///////////////////////////////////////////
					if (mu == 35){
						//KINESIN_ATTACHMENT
						kinesin_attach(mds.h_r, mds.h_type, l, mds.h_dom_center, mds.h_connect_1);
					}
					if (mu == 36){
						//KINSIN_DETACHMENT
						kinesin_detach(mds.h_r, mds.h_type, l, mds.h_dom_center, mds.h_connect_1, mds. h_connect_2, mds.h_len_1, mds.h_len_2, mds.h_att_l);
					}
					if (mu == 37){
						//KINESIN_STEP
						kinesin_step(mds.h_r, mds.h_type, mds.h_length, l, mds.h_dom_center, mds.h_connect_1, mds. h_connect_2, mds.h_len_1, mds.h_len_2, mds.h_f_out, mds.h_att_l);
					}
				}
			} 
		}
